  ...
  return {
    ...
    execute: function () {
      ...
      _export("ConvertedClass", ConvertedClass = (_dec = ccclass('ConvertedClass'), _dec(_class = class ConvertedClass extends Component {
        ...
        start() {
          var self = this.node;
          var uiOpacity = self.getComponent(UIOpacity) || self.addComponent(UIOpacity);
          uiOpacity.opacity = 0;
          var seqOpacity = tween(uiOpacity).delay(1).to(6, {
            opacity: 255
          }).union().repeatForever();
          var seqPosition = tween(self).delay(1).by(500, {
            position: new Vec3(1000, self.getPosition().y, 0)
          }).call(() => {
            var contentSize = self.getComponent(UITransform).contentSize;
